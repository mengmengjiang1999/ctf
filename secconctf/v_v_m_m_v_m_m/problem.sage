...
def gen_key():
    ...
    # 相当于生成一系列22-65维的矩阵list Fs，然后和分别和66维、44维的矩阵组成private key
    # public key是先对这个Fs中的每一个矩阵都进行正交变换，再和44维的矩阵做张量积
    pubkey = SFSs
    pubkey = product(T, SFSs)

    return (pubkey, (S, T, Fs))

# 待加密消息是vector，加密就是对公钥中的每个矩阵跟vector进行一些乘积操作
def eval_v(pubkey, v):
