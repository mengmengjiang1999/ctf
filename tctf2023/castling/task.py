...
class thisishash():
    ...
    def hash(self, msg):
        ...
        msg += len(msg).to_bytes(int(log2(len(msg))),byteorder='little')
        while len(msg)%8 != 0:
            msg += b'\x00'
        text = b'\x00'*8
...
class Task(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        self.request.sendall(b"first message(hex): ")
        msg0 = self.recvhex(2049)
        self.request.sendall(b"second message(hex): ")
        msg1 = self.recvhex(2049)
        if msg0!=msg1 and self.dohash(msg0) == self.dohash(msg1):
            # self.request.sendall(flag.encode('latin-1')+b'\n')
            self.request.sendall("23333")
        self.request.close()
