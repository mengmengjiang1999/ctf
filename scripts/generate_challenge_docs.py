...
""" + "\n".join(rows) + "\n"
    ...
    for artifact in sorted(ROOT.rglob("*"), key=lambda p: str(p.relative_to(ROOT)).casefold()):
        ...
        if relative.parts[0] in {".git", "docs", "scripts"}:
            continue
        if artifact.name.casefold() == "readme.md" or artifact.name == ".gitignore":
            continue
    ...
    checksum_header = (
        "# SHA-256 manifest for historical artifacts.\n"
        "# Generated by scripts/generate_challenge_docs.py; README/docs/scripts are excluded.\n"
    )
